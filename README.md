...
The layout of the demo is intended to be fairly basic.  For application-specific files, `.c` files are stored in the `src` directory, while `.h` files are stored in the `include` directory.  `main.c` contains the initialization code and handles the LED and button interactions.  `const_files.h`, `fs_utils.c/.h`, and `reach-server.h` are also manually generated.  All other files are auto-generated by the scripts found in `reach-util/c-gen`, with application-specific code also contained within these files.  Generally, there is one file per Reach service, as well as the central `device.c/.h` files.  These files are auto-generated using the `update_definitions.bat` script based on the `Reach nRF52840 Dongle.json` specification file.  For more information on using the code generator, see `reach-util/c-gen/README.md`.
